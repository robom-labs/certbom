...
test("출처 규정을 확인하지 못한 시험은 구체적인 일반 준비 체크리스트를 표시한다", async ({ page }) => {
  await page.goto("/#exam/itq?from=find");
  await expect(page.getByText("공식 세부 준비물은 확인 중이지만, 빈손으로 돌려보내지 않아요.")).toBeVisible();
  await expect(page.locator(".checklist label")).toHaveCount(7);
  await expect(page.getByText("일반 준비", { exact: true })).toHaveCount(7);
  await expect(page.getByRole("checkbox", { name: /^사진이 있는 신분증/ })).toBeVisible();
  await expect(page.getByRole("checkbox", { name: /^실기 도구/ })).toBeVisible();
});
...
test("설정에서 다섯 패밀리 앱과 게스트·지원·개인정보·현재 메타를 확인한다", async ({ page }) => {
  ...
  await expect(page.getByText("0.7.1", { exact: true })).toBeVisible();
