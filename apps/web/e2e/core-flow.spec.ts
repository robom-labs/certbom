...
test("홈 세 요약 버튼이 URL 상태와 바로 아래 실제 목록을 바꾼다", async ({ page }) => {
  ...
  // 홈 요약 목록은 HomeScreen에서 slice(0, 8)로 8개까지만 노출하고 나머지는 "시험 찾기에서 N개 모두 보기"
  // 버튼으로 넘긴다. 접수 가능 시험이 8개를 넘으면(실데이터가 자주 그렇다) 목록은 8개까지만 보이므로,
  // 배지의 전체 건수가 아니라 노출 상한을 반영해 비교한다(이 전제를 안 지키면 데이터가 8을 넘을 때마다 실패).
...
test("설정에서 다섯 패밀리 앱과 게스트·지원·개인정보·현재 메타를 확인한다", async ({ page }) => {
  ...
  await expect(page.getByText("0.7.5", { exact: true })).toBeVisible();
