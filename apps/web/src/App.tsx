...
export function App() {
  ...
  const renderContent = () => {
    if (route.kind === "exam") {
      ...
      if (!exam) return <FindScreen favorites={favorites.ids} onOpen={openExam} onToggleFavorite={favorites.toggle} />;
      return <DetailScreen exam={exam} favorite={favorites.ids.includes(exam.id)} checkedIds={checked.ids} onBack={() => navigateTab(route.from)} onToggleFavorite={favorites.toggle} onToggleChecked={checked.toggle} />;
    }
    if (route.tab === "find") return <FindScreen favorites={favorites.ids} startRecommend={route.recommend} onOpen={openExam} onToggleFavorite={favorites.toggle} />;
    if (route.tab === "schedule") return <ScheduleScreen favoriteIds={favorites.ids} onFind={() => navigateTab("find")} onOpen={openExam} />;
    if (route.tab === "settings") return <SettingsScreen favoriteCount={favorites.ids.length} onClear={() => { favorites.clear(); checked.clear(); }} />;
    return <HomeScreen favorites={favorites.ids} onFind={() => navigateTab("find")} onRecommend={() => navigateTab("find", true)} onOpen={openExam} onToggleFavorite={favorites.toggle} />;
  };
